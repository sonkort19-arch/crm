...
- **Администратор по умолчанию:** логин `560676`, пароль `5560676` (при каждом запуске сервера запись администратора в БД приводится к этим значениям).
- Сотрудники из демо-данных: логины `yuri`, `alex`, `erika`, пароль **`1234`** (миграция задаёт их при обновлении схемы).

Токен и профиль (без пароля) хранятся в `localStorage`: ключи `token`, `crm_auth_user`.
