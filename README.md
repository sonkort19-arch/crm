...
## Новые инструменты оператора

- **Снимок состояния** / **Откат к последнему снимку** в разделе «Зарплата».
- **Audit trail** в данных (`salary.auditLog`) для ключевых действий (начисления, выплаты, удаление выплат, импорт процентов, экспорт истории).
- **Фильтры истории**: тип операции, минимальная сумма, быстрый поиск.
- **Экспорт CSV** из истории сотрудника по выбранному периоду и фильтрам.
